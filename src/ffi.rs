...
extern {
    ...
    pub fn ntru_gen_key_pair(params: *const NtruEncParams, kp: *mut NtruEncKeyPair,
                             rand_ctx: *const CNtruRandContext) -> uint8_t;
    pub fn ntru_encrypt(msg: *const uint8_t, msg_len: uint16_t, public: *const NtruEncPubKey,
                        params: *const NtruEncParams, rand_ctx: *const CNtruRandContext,
    ...
    pub fn ntru_rand_init(rand_ctx: *mut CNtruRandContext, rand_gen: *const NtruRandGen)
                          -> uint8_t;
    pub fn ntru_rand_init_det(rand_ctx: *mut CNtruRandContext, rand_gen: *const NtruRandGen,
                              seed: *const uint8_t, seed_len: uint16_t) -> uint8_t;
    pub fn ntru_rand_generate(rand_data: *mut uint8_t, len: uint16_t,
                              rand_ctx: *const CNtruRandContext) -> uint8_t;
    pub fn ntru_rand_release(rand_ctx: *mut CNtruRandContext) -> uint8_t;

    #[cfg(target_os = "windows")]
    pub fn ntru_rand_wincrypt_init(rand_ctx: *mut NtruRandContext, rand_gen: *const NtruRandGen)
                                   -> uint8_t;
    #[cfg(target_os = "windows")]
    pub fn ntru_rand_wincrypt_generate(rand_data: *mut uint8_t, len: uint16_t,
                                       rand_ctx: *const NtruRandContext) -> uint8_t;
    #[cfg(target_os = "windows")]
    pub fn ntru_rand_wincrypt_release(rand_ctx: *mut NtruRandContext) -> uint8_t;

    #[cfg(not(target_os = "windows"))]
    pub fn ntru_rand_devrandom_init(rand_ctx: *mut CNtruRandContext, rand_gen: *const NtruRandGen)
                                    -> uint8_t;
    #[cfg(not(target_os = "windows"))]
    pub fn ntru_rand_devrandom_generate(rand_data: *mut uint8_t, len: uint16_t,
                                        rand_ctx: *const CNtruRandContext) -> uint8_t;
    #[cfg(not(target_os = "windows"))]
    pub fn ntru_rand_devrandom_release(rand_ctx: *mut CNtruRandContext) -> uint8_t;

    #[cfg(not(target_os = "windows"))]
    pub fn ntru_rand_devurandom_init(rand_ctx: *mut CNtruRandContext, rand_gen: *const NtruRandGen)
                                     -> uint8_t;
    #[cfg(not(target_os = "windows"))]
    pub fn ntru_rand_devurandom_generate(rand_data: *mut uint8_t, len: uint16_t,
                                         rand_ctx: *const CNtruRandContext) -> uint8_t;
    #[cfg(not(target_os = "windows"))]
    pub fn ntru_rand_devurandom_release(rand_ctx: *mut CNtruRandContext) -> uint8_t;

    pub fn ntru_rand_igf2_init(rand_ctx: *mut CNtruRandContext, rand_gen: *const NtruRandGen)
                               -> uint8_t;
    pub fn ntru_rand_igf2_generate(rand_data: *mut uint8_t, len: uint16_t,
                                   rand_ctx: *const CNtruRandContext) -> uint8_t;
    ...
    pub fn ntru_rand_tern(n: uint16_t, num_ones: uint16_t, num_neg_ones: uint16_t,
                          poly: *mut NtruTernPoly, rand_ctx: *const CNtruRandContext) -> uint8_t;
