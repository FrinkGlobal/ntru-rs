...
impl Default for Poly {
    fn default() -> Self {
        Poly { tern: TernPoly::default() }
    }
}

impl Poly {
    /// Create a new union from a `ProdPoly`
    fn new_from_prod(prod: ProdPoly) -> Poly {
        Poly { prod }
    }

    /// Create a new `Poly` from a `TernPoly`
    fn new_from_tern(tern: TernPoly) -> Poly {
        Poly { tern }
    }
